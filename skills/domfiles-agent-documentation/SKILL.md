...
- Treat the global “Writing” policy as the canonical owner of the required standalone mirrors in the `human-facing-writing` skill’s `references/typography.md`.
...
- When authoring, reviewing, auditing, or maintaining a public skill, follow the [public skill portability contract](references/public-skill-portability.md). When its top-level `SKILL.md` is in scope, also apply `human-facing-writing` under that contract.
