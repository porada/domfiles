...
Choose references for the relationship they establish, not to accumulate links. Verify that the relevant content supports the claim. Briefly identify how the contribution continues or corrects prior work. For a narrower contribution derived from a closed proposal, identify what it carries forward. Put supporting references beside the constraint or motivation they substantiate, unless the required template places them elsewhere.
...
Use the resolved writing workflow for reference formatting. Without that peer, prefer bare `#<number>` references for same-repository discussions, issues, and pull requests and qualified `<owner>/<repository>#<number>` references across repositories where supported. State relationships in the surrounding prose rather than descriptive Markdown links. Use explicit links for destinations or surfaces that do not support reference shorthand. Use comment or review permalinks only when the reader needs the exact location, not merely because they supplied research evidence. Keep precise research citations in the supporting evidence without automatically carrying them into the final copy.
