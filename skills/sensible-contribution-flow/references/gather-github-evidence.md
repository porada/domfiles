...
After a retrieval failure, stop retrieving that resource. Report the resource, attempted method, exact error, and smallest corrective action. Redact any secret-bearing detail rather than reproducing it, and identify the redaction. Do not retry through another agent, browser, proxy, or tool. Correct an ordinary path or URL mistake and retry only the selected method. Authentication, network, permission, and unexplained transport errors are not empty search results or permission to switch interfaces.
