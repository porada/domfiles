...
Record the fetched target tip, then plan branch setup under its applicable authority. Select or retain the [contribution branch](#select-branch-and-scope), recording its identity and authorized branch setup or replay effects before creating it or changing history. [Synchronize](#synchronize-with-upstream) using that fetched tip, supplying the completed authorization record to the resolved [commit workflow](../SKILL.md#compose-with-peers) for necessary history updates and retaining any limits on base-only replay.

A direct upstream clone supports authorized local preparation without a fork. Defer publication destination setup to [Resolve Publication Destination](#resolve-publication-destination), but collect destination evidence earlier whenever the commit workflow needs it for rewrite eligibility or preservation of published work.
...
Branch updates are confined to the contribution branch. Do not rebase, merge into, reset, or synchronize a repository’s primary branch as a substitute. If the resolved commit workflow cannot establish eligibility or complete the approved update, report the remaining synchronization requirement rather than claiming readiness or selecting another rewrite method.
...
Reassess the body’s [reference relationships](prepare-post-content.md#select-references) against the final outcome. Report material evidence or validation limitations without requiring an external review. An amendment or rebase invalidates the earlier final readiness check, so reassess the affected result. Complete [Resolve Publication Destination](#resolve-publication-destination) when publication is needed. When a published branch requires authorized replacement, have the resolved commit workflow recheck the publication destination immediately before handoff, reconcile changed remote work within authorization, and repeat affected validation. New scope requires a user decision, not merely a newer lease expectation. Then return to the entrypoint’s complete [handoff](../SKILL.md#hand-back-the-contribution).

## Resolve Publication Destination

Run this checkpoint after the authorized local PR work is complete and before its publication handoff. Assessment-only and working-tree-only deliverables do not require it.

1. Preserve an established publication destination, especially an existing PR’s head repository. Otherwise consider the verified upstream repository as the candidate. Write access determines where the contribution branch can be published, not whether a pull request is required.
2. Use the resolved evidence workflow to verify write access for the intended operation on the contribution branch from repository permission metadata and applicable branch restrictions. Do not infer access from ownership, fork status, or remote names, and never probe by pushing. Unavailable or inconclusive evidence is an evidence gap, not confirmed lack of access. Follow the evidence workflow’s retrieval boundaries rather than treating a lookup failure as a reason to fork.
3. If the candidate is writable, use it. Only after confirmed lack of access, consider another destination. Changing an established destination requires a user decision, especially when it would replace an existing PR. Otherwise reuse a suitable existing fork whose upstream relationship and required branch access are verified.
4. When confirmed lack of access leaves a new PR needing a fork, present the prepared local result. If the already resolved local `simple-github-cli` peer is available, ask for explicit permission covering the verified source repository, concrete fork destination, and any necessary local remote setup. After approval, use that peer to create only the hosted fork, without cloning, publishing local refs, or implicitly changing remotes. If the peer is unavailable, ask the user to create the fork and supply its URL instead.
5. Verify any created or user-supplied fork’s upstream relationship and required branch access before preparing a publication command. Preserve the supplied checkout and existing remotes, and perform only authorized local setup. If approval is declined or required evidence or capabilities remain unavailable, hand back the prepared local result with the exact remaining requirement rather than claiming publication readiness. Git publication and PR submission remain [user-run](../SKILL.md#hand-back-the-contribution).
