...
5. **Mutation and approval:** Trace every local mutation, dependency change, remote effect, publication step, and user-only operation to the actor, required authorization, terminal action, or required stop. For each dependency change, also inventory its selection and informed-approval requirements. Keep drafting, review, preparation, and tool availability from implying authority for a later effect.
