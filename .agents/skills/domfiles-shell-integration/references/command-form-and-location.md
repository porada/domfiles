...
- After that check, default a command used only through Fish to `home/.config/fish/functions/<command-name>.fish`.
...
- Add `home/.local/bin/<command-name>` only when the selected execution boundary requires an executable, including invocation by a non-Fish caller or before Fish is available.
- Put a `domfiles` subcommand at `home/.local/bin/domfiles-<command-name>`. For a Git subcommand, follow [Git helper form](#choose-git-helper-form) before using `home/.local/bin/git-<command-name>`.

## Choose Git Helper Form

- Before adding or reviewing a `home/.local/bin/git-*` entrypoint, inspect `home/.config/git/config` and determine whether a plain Git alias preserves the required behavior.
...
- If a Git alias would require the `!` shell-command form, implement it as a `home/.local/bin/git-*` script instead. Never define shell commands inside `home/.config/git/config` aliases.
