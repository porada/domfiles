...
- Before adding or reviewing a `bin/git-*` entrypoint, inspect `home/.config/git/config` and determine whether a plain Git alias preserves the required behavior.
...
- If a Git alias would require the `!` shell-command form, implement it as a `bin/git-*` script instead. Never define shell commands inside `home/.config/git/config` aliases.
