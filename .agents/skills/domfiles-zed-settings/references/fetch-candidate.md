...
A scope-only candidate requires no generic terminal validation results. Create an owner-specification file containing `{"owners":[],"overlaps":[]}` and, separately, a candidate validation manifest with an empty `results` array. Then continue with [Build and promote a permission candidate](permission-candidate.md#build-and-promote-a-permission-candidate) from sealing through preflight, approval, promotion or refresh, and post-promotion checks.
