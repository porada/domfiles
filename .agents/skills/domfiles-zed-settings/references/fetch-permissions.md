...
After a `network_hosts`-only change or a fetch pattern or default change, verify the approved exact and wildcard host-grant coverage, complete-array ordering, participating settings layers, and effective fetch and terminal boundaries.
