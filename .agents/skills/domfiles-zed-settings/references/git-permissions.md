...
- Order each permission bucket by section: root forms, per-subcommand discovery forms, general-scope direct and compound workflow groups, then agent-namespace direct and compound workflow groups. `always_confirm` and `always_deny` carry only the general-scope section. Alphabetize by subcommand within each section independently, so one subcommand holds entries in several sections rather than one contiguous group. Assign a pattern to the agent-namespace section by the target its operation acts on rather than by any `.agent-<name>` mention, so an input path supplied through an option value such as `-F` or `--output` stays in the general-scope section. See [Zed Git permission ordering](../../../PROJECT.md#zed-git-permission-ordering) for the constraint this places on an owner-audit manifest.
...
- Keep an operand that directly encodes an explicit network destination confirmable. A URL scheme or scp-style `<host>:<path>` operand reaches an arbitrary host, so a subcommand whose broad allowance accepts remote operands retains that allowance and a separate confirmation recognizes those operand forms. See [Zed Git remote URL scope](../../../PROJECT.md#zed-git-remote-url-scope) for the rationale.
...
- Section placement, alphabetic owner-group ordering within each section, and byte-identical repeated prefix grammar.
