...
Use `.agents/skills/domfiles-zed-settings/scripts/zed-regex-audit.rs` to compare the exact root `Cargo.toml` pin, the `regex` version resolved through the local root package’s adjacent `Cargo.lock` dependency reference, and the `regex` version in the retrieved upstream lockfile. It compares only the direct `regex` version; transitive package versions, sources, checksums, and dependency edges remain independently updatable. It reads local files only and never retrieves upstream source or modifies repository state. Run it with `cargo run --locked --quiet --bin domfiles-zed-settings-zed-regex-audit -- --local-manifest Cargo.toml --upstream-lock <zed-checkout>/Cargo.lock --upstream-revision <short-zed-revision>`. Its focused contract test is defined in [Compile permission patterns](#compile-permission-patterns).

Treat a direct-version mismatch as a semantic-accuracy finding against the documented compatibility claim. Treat a missing or ambiguous `regex` package, an unresolved local root dependency reference, or a local manifest and lockfile version mismatch as a validation failure because the direct version cannot be established. Matching exact versions satisfies compatibility regardless of transitive dependency graph differences. Neither result authorizes a dependency or documentation change.
