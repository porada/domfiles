...
3. Write one pattern and one complete raw or normalized input to separate files under the current task’s `.agent-<name>` directory. Do not append synthetic line terminators or combine multiple inputs in one file.
...
1. Build the effective permission settings.
    - Accumulate `always_deny`, `always_confirm`, and `always_allow` patterns across user, project, and profile settings. A higher-priority layer cannot remove an accumulated pattern.
...
Use the [bounded smoke-test workflow](#smoke-test-permission-patterns) for eligible configured patterns and independently evaluated inputs. For every ineligible case, consult current Zed source or verify behavior in a running Zed instance instead of inferring Zed behavior from `rg`.
