...
The global [`repository-harmonization` skill](../skills/domfiles-repository-harmonization/SKILL.md) is invoked explicitly as `/repository-harmonization` and owns its change-oriented cross-repository consistency workflow. Its `disable-model-invocation: true` frontmatter prevents automatic model invocation.

### Shorthand command routing

Under the [skill-description invocation modes](../skills/domfiles-agent-documentation/SKILL.md#compose-the-change), a model-invocable skill’s description routes any bare shorthand by declaring the command. The global [shorthand-command policy](../.config/zed/AGENTS.md#shorthand-commands) therefore carries no per-skill route headings. A heading would duplicate a trigger the description already provides and would require maintenance for every current and future shorthand. A command-only skill instead uses its installed name as an explicit slash command. `Verify` remains defined inline because no skill owns it.
