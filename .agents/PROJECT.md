...
[`bin/domfiles-sync-setup`](../bin/domfiles-sync-setup) defines the exact source-to-destination mappings for globally exposed skills. The current `skills/domfiles-*` set is global, while `skills/human-facing-writing` is public.
...
Public skill descriptions are human-facing discovery and marketing surfaces as well as routing metadata, so their composition follows the [public skill portability contract](../skills/domfiles-agent-documentation/references/public-skill-portability.md). Their YAML `|-` literal block scalars preserve paragraph boundaries in decoded metadata while removing the trailing newline. A client may fold those boundaries in a list view without changing the description contract.
