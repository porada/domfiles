...
- **Security boundaries:** Use supported permission and sandbox-grant mechanisms. Never disable, weaken, or bypass agent approvals, sandboxing, hook trust, operating-system trust or quarantine, TLS verification, package integrity or trust checks, credential protection, or comparable security controls. Never change execution identity or privilege, and never enable automatic loading of untrusted working-directory startup or debugger configuration. If a task requires such a boundary change, provide instructions for the user instead of performing it.
...
- **Review convergence:** After a second consecutive review round finds new issues in the same example, rule, or implementation, stop applying local fixes and reassess whether the construct should be narrowed, replaced, or removed. When the current task already authorizes working-tree fixes, resolve the resulting finding batch in the working tree, then run one bounded adversarial review of the complete in-scope, task-owned diff. Otherwise, report the finding batch without editing. Do not update a reviewed commit after each individual fix, and leave authorized fixes in the working tree until any commit update is authorized through the Commit gate.
...
- **Availability:** Assume these non-standard development commands are installed and available through `PATH`: `actionlint`, `agent-browser`, `ast-grep`, `cargo`, `fd`, `fish`, `gh`, `jq`, `just`, `node`, `pandoc`, `plugins`, `pnpm`, `rg`, `rustc`, `shellcheck`, `skills`, `taplo`, `yarn`, `yq`, and `zizmor`.
...
- **Browser automation:** Prefer `agent-browser` over Chrome MCP and invoke it directly. The domfiles shim supplies managed global configuration and bypasses upstream automatic discovery of `~/.agent-browser/config.json` and `./agent-browser.json`. When project-local configuration is required, use `agent-browser --config ./agent-browser.json <command> …`. This is the only supported local-config route. The shim rejects inherited `AGENT_BROWSER_*` values that select external state or hidden execution inputs and directs callers to visible CLI options.
...
- **Permission boundary:** Treat permission prompts as security boundaries, not command-syntax problems. Do not inspect or reverse-engineer editor permission settings unless the task explicitly concerns them. Do not change tools or reformulate an operation merely to obtain a less restrictive decision. When a necessary, task-authorized operation requires a grant, request the narrowest grant once with a concise reason.
