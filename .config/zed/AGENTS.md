...
- **Browser automation:** Load the `agent-browser` skill before using `agent-browser`, invoke it directly, and prefer it over Chrome MCP. The domfiles shim supplies managed global configuration and bypasses upstream automatic discovery of `~/.agent-browser/config.json` and `./agent-browser.json`. When project-local configuration is required, use `agent-browser --config ./agent-browser.json <command> …`. This confirmed form is the only supported local-config route. The shim rejects inherited `AGENT_BROWSER_*` values for confirmed capabilities and directs callers to visible CLI options.
- **Fish:** Default to `fish --no-config` when using Fish as a noninteractive interpreter. Set `MANPAGER=cat` and `PAGER=cat` when requesting Fish-owned help. Use `builtin <name>` inside evaluated command text when targeting a Fish builtin. Omit `--no-config` when Fish startup configuration or configured runtime behavior is in scope.
...
- **Pandoc selection:** Run `pandoc` only when the task requires Pandoc-specific behavior or after establishing that no available native or dedicated tool can perform the required document operation. Installation and availability alone do not satisfy this gate.
- **Pandoc invocation:** Always write long options that take values with `=`, as in `--from=gfm`. Never separate an option from its value, as in `--from gfm`. Use `pandoc --sandbox` for document parsing and conversion when the reader and writer need access only to files supplied on the command line. Treat `--sandbox` as a reader and writer boundary only—it does not constrain filters or PDF engines.
